...
def proccess_cnn(url, model):
    ...
    padded_data = tokenData + [0] * (64 - len(tokenData))
...
def url_final_result(url, model1, model2, model3, model4, model5, model6, model7):
    ...
    for model in [model1, model2, model3, model4, model5, model6, model7]:
        ...
        if tmp_answer == 0:
            answer = 'safe'
            prob = "{:.4f}".format(predicted_probabilities.flatten()[0])
            tmp_data.append([answer, float(prob)])
        elif tmp_answer == 1:
            answer = 'malicious'
            prob = "{:.4f}".format(predicted_probabilities.flatten()[1])
            tmp_data.append([answer, float(prob)])

    # dga까지 tmp_data에 추가!
    tmp_data.append(proccess_cnn(url, model_cnn))
    # 임시 출력
    print(tmp_data)
    ...
    for i in range(len(tmp_data)):
        if tmp_data[i][0] == 'safe':
            ...
            benign_prob += weights[i] * float(tmp_data[i][1])
            all_benign_prob += all_weights[i] * float(tmp_data[i][1])
        else:
            all_malicious_count += all_weights[i]
            malicious_count += weights[i]
            malicious_prob += weights[i] * float(tmp_data[i][1])
            all_malicious_prob += all_weights[i] * float(tmp_data[i][1])

    if all_benign_count >= all_malicious_count:
        # AI값 저장
        if benign_count >= malicious_count:
            AI_answer = 'safe'  # AI의 답
            AI_prob = "{:.4f}".format(benign_prob / benign_count * 100)
        else:
            AI_answer = 'malicious'
            AI_prob = "{:.4f}".format(malicious_prob / malicious_count * 100)

        # DGA값 저장
        DGA_answer = tmp_data[7][0]  # DGA정답
        DGA_prob = "{:.4f}".format(tmp_data[7][1] * 100)

        # 전체 확률 및 신뢰도 저장
        Total_answer = 'safe'  # 전체정답
        Total_prob = "{:.4f}".format(all_benign_prob / all_benign_count * 100)
        Truth_prob = "{:.4f}".format(all_benign_count * 100)
        ...
        Total_prob = "{:.4f}".format(all_malicious_prob / all_malicious_count * 100)
        Truth_prob = "{:.4f}".format(all_malicious_count * 100)
...
# 모델 불러오기
model1 = joblib.load('./models/model_RF.h5')
model2 = joblib.load('./models/model_GB.h5')
model3 = joblib.load('./models/model_HGB.h5')
model4 = joblib.load('./models/model_1.h5')
model5 = joblib.load('./models/model_2.h5')
model6 = joblib.load('./models/model_3.h5')
model7 = joblib.load('./models/model_4.h5')
model_cnn = keras.models.load_model("./models/CNN.h5")
...
#########################################################
    ...
    if text == "HOME":
        ...
        csv_file = "./print.csv"  # 저장한 CSV 파일 경로를 지정하세요.
...
#######################################################
    # 2번째 탭, 설명탭
        ...
        image = Image.open("./img/그림01.png")  # 동작 방식을 위한 이미지 불러오기
...
# 학교 로고 이미지를 불러와 표시
icon = ImageTk.PhotoImage(file="./img/학교로고.png")
